...
namespace Common
{
    public class BaseDataAccess
    {
        ...
        /// 分页SQL执行用
        /// </summary>
        /// <param name="inBaseBean">基础信息：画面信息</param>
        /// <param name="inCols">列</param>
        /// <param name="inTables">表</param>
        /// <param name="inOrder">显示順</param>
        /// <param name="inCondition">检索条件</param>
        /// <returns>执行結果</returns>
        public DataTable ExecutePagingProc(
            BasePagingBean inBaseBean, string inCols, string inTables, string inOrder, string inCondition)
        {

            DataTable returnValue = null;

            //执行存储过程
            string procName = "SP_WEB_PAGING";
            Int32 outPages = 0;
            Int32 outRecordCount = 0;

            //检索条件设定和参数的设定
            SqlParameter[] paras = new SqlParameter[8];

            //表名
            paras[0] = new SqlParameter("@TABLENAMES", inTables);
            //检索项目
            paras[1] = new SqlParameter("@COLUMNS", inCols);
            //检索順
            paras[2] = new SqlParameter("@ORDERCOLUMNNAME", inOrder);
            //检索条件
            paras[3] = new SqlParameter("@WHERE", inCondition);
            //当前数据的显示されている画面：默认：１
            paras[4] = new SqlParameter("@PAGECURRENTINDEX", inBaseBean.PageIndex);
            //1画面的数据数：系统控件信息
            paras[5] = new SqlParameter("@PAGEEACHSIZE", inBaseBean.PageSize);
            //输出数据：画面数
            SqlParameter outPPages = new SqlParameter("@PAGECOUNT", outPages);
            outPPages.Direction = ParameterDirection.Output;
            paras[6] = outPPages;
            //输出数据：結果件数
            SqlParameter outPRecordCNT = new SqlParameter("@ITEMSCOUNT", outRecordCount);
            outPRecordCNT.Direction = ParameterDirection.Output;
            paras[7] = outPRecordCNT;

            // 执行前，在LOG中输出SQL文。
            LogUtil.WriteDebugMessage(this.GetLogSQLProc(procName, paras));
            returnValue = this.ExecuteProcWithDataTable(procName, paras);
            //結果数和画面数
            inBaseBean.PageCount = Convert.ToInt32(paras[paras.Length - 2].Value);
            inBaseBean.ItemsCount = Convert.ToInt32(paras[paras.Length - 1].Value);

            // 执行后，在LOG中输出执行结果。
            LogUtil.WriteDebugMessage(this.GetExecuteLog(returnValue.Rows.Count));
