            ...
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    for (int i = 0; i < fileRows.Count; i++)
                    {
                        ...
                        // 业务参考号 否
                        //pay.YURREF = DateTime.Now.ToString("yyyyMMdd") + arr[4];    //Customer Reference
                        // 期望日
                        ...
                        pay.STLCHN = "N";
                        // 用途 否
                        pay.NUSAGE = SystemInfoController.GetControlInfo(Constant.SystemControlInfo.KEY_USAGE).ItemValue;
                        // 业务摘要
                        pay.BUSNAR = arr[56];
                        // 收方帐号 否
